...
export async function POST(req: NextRequest) {
  ...
  WHERE id = ${id}
  RETURNING id, name, email, date, time, note;
`;
...
const formattedDate = `${rawDate.getDate().toString().padStart(2,'0')}.${(rawDate.getMonth()+1).toString().padStart(2,'0')}.${rawDate.getFullYear()}`;
    ...
    await transporter.sendMail({
      ...
      subject: `Ihr Termin konnte nicht bestätigt werden`,
  html: `
<div style="max-width: 600px; margin: 0 auto; padding: 20px;
  ...
  <div style="background: #ffffff; padding: 20px 24px;
    ...
    <h2 style="margin-top: 0; color: #dc3545; font-size: 22px;">
    ...
    <p style="font-size: 15px; line-height: 1.6; margin: 0 0 12px;">
      Leider kann Ihr Termin am <strong>${formattedDate}</strong> um <strong>${time}</strong> Uhr nicht bestätigt werden.
      
    </p>

    <p style="font-size: 15px; line-height: 1.6; margin: 0 0 12px;">
      Bitte versuchen Sie, einen alternativen Termin über unser Buchungssystem zu vereinbaren.
    </p>
